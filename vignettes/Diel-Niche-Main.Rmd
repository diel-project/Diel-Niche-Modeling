...
The best way to understand each hypothesis set is by plotting hypotheses from a set. For the three fundamental sets:
...
plot.diel(hyp=hyp.sets("Selection"),diel.setup = diel.ineq(p.avail=c(0.04,0.48)))
```
...
plot.diel(hyp=hyp.sets("Selection"),diel.setup = diel.ineq(p.avail=c(0.04,0.3)))
```
